...
type MinecraftService interface {
	...
	GetPlaytimeByMinecraftUUIDs(ctx context.Context, mcUUIDs uuid.UUIDs) (map[uuid.UUID]*domain.Playtime, error)
...
func (s *minecraftService) GetPlaytimeByMinecraftUUIDs(ctx context.Context, mcUUIDs uuid.UUIDs) (map[uuid.UUID]*domain.Playtime, error) {
	playtimes, err := s.shellAPI.GetPlaytime(ctx, mcUUIDs)
	if err != nil {
		return nil, utils.NewInternalServerError("failed to get playtime by minecraft uuids", err)
	}
