...
// worldNamePattern is the shape of a squaremap world name, e.g. minecraft_overworld.
var worldNamePattern = regexp.MustCompile(`^[a-z0-9_]{1,64}$`)
...
// ClaimChunksCommand claims or releases chunks of one world. ProfileID is the profile acting on its own claims;
// it is uuid.Nil for an admin releasing whoever holds the chunks.
...
func (c *ClaimChunksCommand) Validate() error {
	return validation.ValidateStruct(c,
		validation.Field(&c.World, validation.Required, validation.Match(worldNamePattern)),
